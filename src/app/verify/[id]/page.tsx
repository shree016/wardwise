...
export default function VerifyPage() {
  ...
  const getVerifierLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        setVerifierLocation({ lat: position.coords.latitude, lng: position.coords.longitude });
  ...
  const handleVerify = async () => {
    if (!image || !issue || !verifierLocation) return;
    setVerifying(true);
    try {
      const res = await axios.post(`/api/verify/${id}`, {
        after_photo: image,
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
          ...
          {['Phone OTP', 'Location', 'Photo', 'Submit'].map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                ...
                {(i === 0 && otpVerified) || (i === 1 && verifierLocation) || (i === 2 && image) ? '✓' : i + 1}
              </div>
        ...
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 mb-6">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="font-semibold capitalize text-lg">{issue.issue_type.replace('_', ' ')}</h3>
            ...
            <div className="text-right">
              <span className="bg-yellow-600 text-white text-xs px-3 py-1 rounded-full">{issue.status}</span>
        ...
        {!otpVerified ? (
          <div className="bg-gray-900 rounded-xl p-5 border border-blue-800 mb-6">
            <h3 className="font-semibold text-blue-400 mb-1">Step 1: 📱 Verify Your Identity</h3>
            <p className="text-gray-400 text-sm mb-4">Enter your phone number to receive an OTP</p>
            {!otpSent ? (
        ...
        <div className={`mb-6 ${!otpVerified ? 'opacity-40 pointer-events-none' : ''}`}>
          ...
          <button
            ...
            className={`w-full border px-4 py-3 rounded-xl text-left transition flex items-center gap-3 ${
              verifierLocation ? 'bg-green-900 border-green-600' : 'bg-gray-900 border-gray-700 hover:border-blue-500'
            }`}
          >
            <span className="text-2xl">{locationLoading ? '⏳' : verifierLocation ? '✅' : '📍'}</span>
            <div>
              <div className="text-white text-sm font-medium">
                {locationLoading ? 'Getting your location...' : verifierLocation ? 'Location Detected!' : 'Detect My Location'}
              </div>

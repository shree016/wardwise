  ...
  return (
    ...
    >
      <span className={cn(
        ...
        isDark ? "opacity-100 bg-gradient-to-br from-violet-500/10 to-blue-500/10" : "opacity-100 bg-gradient-to-br from-amber-100/60 to-orange-100/60"
      )} />
      <SunIcon className={cn("absolute w-4 h-4 text-amber-500 transition-all duration-300", isDark ? "opacity-0 rotate-90 scale-50" : "opacity-100 rotate-0 scale-100")} />
      <MoonIcon className={cn("absolute w-4 h-4 text-violet-400 transition-all duration-300", isDark ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-90 scale-50")} />
    </button>
  );
};
...
const Navbar = () => {
  ...
  return (
    <header className={cn(
      ...
      scroll && "border-border/40 bg-background/60 backdrop-blur-xl shadow-sm dark:shadow-[0_1px_30px_rgba(139,92,246,0.06)]"
    )}>
      <AnimationContainer reverse delay={0.1} className="size-full">
        <MaxWidthWrapper className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-8">
            <Link href={isLoggedIn ? "/citizen" : "/"} className="flex items-center gap-2 group">
              ...
              <nav className="hidden lg:flex items-center gap-1">
                {CIVIC_LINKS.map(({ href, label, icon: Icon }) => (
                  <Link key={href} href={href}
                    className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition-all duration-200">
                ...
                <NavigationMenuList>
                  {NAV_LINKS.map((link) => (
                    <NavigationMenuItem key={link.title}>
                      {link.menu ? (
                        <>
                          <NavigationMenuTrigger className="text-sm">{link.title}</NavigationMenuTrigger>
                          <NavigationMenuContent>
                            <ul className={cn("grid gap-1 p-4 md:w-[400px] lg:w-[500px] rounded-xl", link.title === "Features" ? "lg:grid-cols-[.75fr_1fr]" : "lg:grid-cols-2")}>
                              {link.title === "Features" && (
                                <li className="row-span-4 pr-2 relative rounded-lg overflow-hidden">
                                  ...
                                  <NavigationMenuLink asChild className="z-20 relative">
                                    <Link href="/#features" className="flex h-full w-full select-none flex-col justify-end rounded-lg bg-gradient-to-b from-violet-500/10 to-blue-500/10 border border-border/40 p-4 no-underline outline-none focus:shadow-md">
                                      <MapPinIcon className="w-6 h-6 text-violet-400 mb-2" />
                                      <h6 className="mb-2 text-base font-semibold bg-gradient-to-r from-violet-400 to-blue-400 bg-clip-text text-transparent">All AI Features</h6>
                                      <p className="text-sm leading-tight text-muted-foreground">AI detection for potholes, garbage & street lights — with geo-mapping, heatmaps, and smart priority.</p>
                                    </Link>
                                  </NavigationMenuLink>
                                </li>
                              )}
                              {link.menu.map((menuItem) => (
                                <ListItem key={menuItem.title} title={menuItem.title} href={menuItem.href} icon={menuItem.icon}>{menuItem.tagline}</ListItem>
                              ))}
                            </ul>
                          </NavigationMenuContent>
                        </>
                        ...
                        <Link href={link.href} legacyBehavior passHref>
                          <NavigationMenuLink className={cn(navigationMenuTriggerStyle(), "text-sm")}>{link.title}</NavigationMenuLink>
                        </Link>
                      )}
                    </NavigationMenuItem>
                  ))}
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            {isLoggedIn ? (
              <>
                <Link href="/citizen" className={buttonVariants({ size: "sm", variant: "ghost", className: "text-sm gap-1.5" })}>
  ...
  <li>
    <NavigationMenuLink asChild>
      <Link href={href!} ref={ref}
        className={cn("block select-none space-y-1 rounded-lg p-3 leading-none no-underline outline-none transition-all duration-200 ease-out hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground", className)}
        {...props}>
        <div className="flex items-center space-x-2 text-foreground/80">
          <Icon className="h-4 w-4 text-violet-500" />
          <h6 className="text-sm font-medium !leading-none">{title}</h6>
        </div>
        <p title={children! as string} className="line-clamp-1 text-sm leading-snug text-muted-foreground">{children}</p>
      </Link>
    </NavigationMenuLink>
  </li>
));

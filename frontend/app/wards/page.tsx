...
export default function WardsPage() {
  ...
  const computeWardStats = (issues: Issue[]) => {
    const map: Record<string, WardStat> = {};

    // Initialize all wards with 0
    allWards.forEach(w => {
      map[w.name] = {
  ...
  const totalVerified = wardStats.reduce((a, b) => a + b.verified, 0);
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-gray-900 rounded-xl p-5 border border-green-900 text-center">
            <div className="text-3xl font-bold text-green-400">{topWard?.ward_name}</div>
          ...
          <div className="bg-gray-900 rounded-xl p-5 border border-red-900 text-center">
            <div className="text-3xl font-bold text-red-400">{worstWard?.ward_name}</div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {wardStats.slice(0, 3).map((ward, i) => (
            <div key={ward.ward_name}
              ...
              <div className={`text-3xl font-bold mt-2 ${getScoreColor(ward.score)}`}>
                {ward.score}
              </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {sorted.map((ward, i) => (
                <tr key={ward.ward_name}
                  className="border-b border-gray-800 hover:bg-gray-800 transition">
                  <td className="px-5 py-4 text-lg">
                    {getRankEmoji(ward.rank)}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span className={`font-bold text-lg ${getScoreColor(ward.score)}`}>
                      {ward.score}
                    </span>
                  </td>
